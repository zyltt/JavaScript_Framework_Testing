...
dataset_name = 'sinewave' # e.g. 'fashion_mnist'
...
if dataset_name == 'sinewave':
    ...
    test_size, seq_len = 49, 48
    data_test = dataframe.get("sinewave").values[-(test_size + 48):]
    data_windows = []
    for i in range(test_size):
        data_windows.append(data_test[i:i + seq_len])
    data_windows = np.array(data_windows).astype(float).reshape((1,test_size, seq_len, 1))
...
if dataset_name == 'price':
    def get_price_data():
        ...
        trainX = trainX[:240]
        trainX = np.reshape(trainX, (1,240,240,1))
...
if dataset_name!='sinewave' and dataset_name!='price':
    np.savez(str(dataset_dir / "inputs.npz"), x_test[:instance_num])
    np.savez(str(dataset_dir / "ground_truths.npz"), y_test[:instance_num])
else:
    np.savez(str(dataset_dir / "inputs.npz"), x_test[:1])
    np.savez(str(dataset_dir / "ground_truths.npz"), y_test[:1])

print(x_test[:instance_num].shape)
print(y_test[:instance_num].shape)

data = np.load('../Dataset/' + dataset_name + '/inputs.npz')
input_corpus = data[data.files[0]]
print(input_corpus[0][0][0][10])